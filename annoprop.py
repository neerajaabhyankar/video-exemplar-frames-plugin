...
def propagate_annotations_sam2(
    ...
    # ASSUMPTION: all the frames in the view are in the same directory
    frames_dir = os.path.dirname(view.first().filepath)
    propagator.initialize(frames_dir)

    # Register all frames
    def register_sample(sample):
        if sample[input_annotation_field]:
            sample_frame = cv2.imread(sample.filepath)
            propagator.register_source_frame(sample_frame, sample[input_annotation_field])

    view.map_samples(register_sample, num_workers=1, save=True)
