...
class PropagatorSAM2:
    def __init__(self, model_cfg=None, checkpoint=None):
        ...
        # one propagator can handle multiple detections;
        self.setup()
    ...
    def register_source_frame(self, source_filepath, source_detections):
        ...
        Args:
            ...
            source_detections: The detections from source_frame (fo.Detections)
        """
        # Initialize inference state all files in the dir of source_filepath
        frames_dir = os.path.dirname(source_filepath)
        # Sort frames_list numerically to match SAM2's internal ordering
        self.frames_list = sorted(
            [os.path.join(frames_dir, f) for f in os.listdir(frames_dir) if f.endswith(('.png', '.jpg', '.jpeg'))],
            key=lambda x: int(os.path.basename(x).split('.')[0])
        )

        # frames_dir needs to have files with numbers only!? how restrictive.
        self.inference_state = self.sam2_predictor.init_state(frames_dir)
        logger.info(f"Inference state initialized with {len(self.frames_list)} frames")
        if not hasattr(source_detections, "detections"):
            return
        
        source_frame_idx = self.frames_list.index(source_filepath)
        ...
        for detection in source_detections.detections:
            ...
            logger.info(f"Added new bounding box for {detection.label} in frame {source_frame_idx} of {frames_dir}")
            
            # # Get the mask from the source detection
            # source_mask = detection.mask
            # if source_mask is not None:
            #     source_mask_fitted = fit_mask_to_bbox(source_mask, (y2-y1, x2-x1))
            #     # make it relative to the target frame
            ...
            #     )
            #     logger.info(f"Added new mask for {detection.label} in frame {source_frame_idx} of {frames_dir}")
        
        self.propagate_to_all_frames()
    ...
    def propagate_to_all_frames(self):
        ...
        for frame_idx, obj_ids, mask_logits in self.sam2_predictor.propagate_in_video(self.inference_state):
            target_filepath = self.frames_list[frame_idx]
            ...
            for oi, obj_id in enumerate(obj_ids):
                ...
                # )
                # refined_mask_fitted = pred[y1_new:y2_new, x1_new:x2_new]
            ...
            self.preds_dict[self.frames_list[frame_idx]] = fo.Detections(detections=propagated_detections)
